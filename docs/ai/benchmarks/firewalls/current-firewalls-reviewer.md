...
- Physical Design Process: 1. Select specific technologies 2. Identify technical solution based on these technologies (deployment, operations, and maintenance) 3. Design Physical Security measures to support the technical solution 4. Prepare project plans for the implementation
...
Categorized by Development Era
...
- It does not recognize packets being part of an existing connection, is trying to establish a new connection, or is just a rogue packet
- Ex. FTP connections opens new connections to random ports by design
...
- Connections from untrusted networks are routed through an external filtering router
- Connections from the untrusted networks are routed into a separate segment called the DMZ
- Connections from trusted networks are allowed through the DMZ

## BEST PRACTICES FOR FIREWALLS

- BEST PRACTICES FOR FIREWALLS: 1. All traffic from the trusted network is allowed. Filtering and logging of outbound traffic are possible when indicated by specific organizational policies. 2. FW device is never directly accessible from the public network for configuration or management purposes. 3. ICMP should be denied. 4. Remote access to all internal servers from the public networks should be blocked. 5. HTTP traffic should be denied if web services are offered to the internet. 6. Block traffic by default and monitor user access. 7. Establish a firewall configuration change plan. 8. Optimize the firewall rules of your network. 9. Update your firewall software regularly. 10. Conduct regular firewall security audits. 11. Have a centralized management tool for multi-vendor firewalls. 12. Automate the process of firewall updating.
