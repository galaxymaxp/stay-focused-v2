...
Phase 5A uses a personal access token generated by the individual Canvas user
who is connecting. There is no school-wide Canvas token. The token belongs only
to that Canvas user, inherits only that user's permissions, cannot access
school-wide data, and cannot bypass locked, hidden, unpublished, or
permission-restricted content. Capability results from one developer-owned live
validation token, including the 17 courses returned for that token, prove only
that user's visible Canvas capabilities.
...
| Token validity | Profile request with bearer token | 5A | User-generated personal access token | Permission-dependent | Native metadata | No | No | No | Tokens can be revoked after validation and must never be shared between Stay Focused users. |
...
| Future OAuth connection | Canvas OAuth authorization-code flow | Future production authorization | Institution-approved Developer Key and user consent | Permission-dependent | Native credential flow | No | No | No | Not implemented. The Developer Key is controlled by the Canvas institution or administrator. OAuth is required before broad public production authorization. |
