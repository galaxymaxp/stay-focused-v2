...
import { structureCanvasReviewerSources } from "@/lib/canvas-reviewer-sources";

export const runtime = "nodejs";
export const maxDuration = 60;

const MAX_STRUCTURE_BODY_BYTES = 32 * 1024;
const STRUCTURE_REQUEST_FIELDS = new Set(["sourceIds"]);

interface CourseSourceStructureRouteContext {
  readonly params: Promise<{ readonly courseId: string }>;
}

export async function POST(
  request: Request,
  context: CourseSourceStructureRouteContext,
): Promise<Response> {
  ...
  const parsed = await readStructureRequest(request);
  if (!parsed.ok) {
    return jsonResponse(
      {
        ok: false,
...
async function readStructureRequest(request: Request): Promise<
...
> {
  ...
  if (isOversizedContentLength(request.headers.get("content-length"))) {
    return {
      ...
      message: `Request body must be at most ${MAX_STRUCTURE_BODY_BYTES} bytes.`,
    };
  }

  const rawBody = await request.text();
  if (new TextEncoder().encode(rawBody).byteLength > MAX_STRUCTURE_BODY_BYTES) {
    return {
      ok: false,
      status: 413,
      code: "payload_too_large",
  ...
  if (!Object.keys(parsed).every((key) => STRUCTURE_REQUEST_FIELDS.has(key))) {
  ...
  if (parsed.sourceIds.some((entry) => typeof entry !== "string")) {
    return invalidRequest("sourceIds must contain Canvas source descriptor IDs.");
  }
...
function isOversizedContentLength(contentLength: string | null): boolean {
  ...
  return Number.isFinite(parsed) && parsed > MAX_STRUCTURE_BODY_BYTES;
}
